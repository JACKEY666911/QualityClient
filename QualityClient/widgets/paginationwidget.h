...
class PaginationWidget : public QWidget {
  Q_OBJECT
  Q_PROPERTY(int totalCount READ totalCount WRITE setTotalCount NOTIFY
                 totalCountChanged)
  Q_PROPERTY(
      int pageSize READ pageSize WRITE setPageSize NOTIFY pageSizeChanged)
  Q_PROPERTY(int currentPage READ currentPage WRITE setCurrentPage NOTIFY
                 currentPageChanged)
  Q_PROPERTY(
      QList<int> pageSizeOptions READ pageSizeOptions WRITE setPageSizeOptions)
...
private:
  ...
  // 更新页码信息显示
  void updatePageInfo();
  void setPageSizeSelectorVisible(bool visible);

private:
  // 分页参数
  int m_totalCount = 0;                            // 总条数
  int m_pageSize = 12;                             // 每页条数
  int m_currentPage = 1;                           // 当前页
  int m_totalPage = 0;                             // 总页数
  QList<int> m_pageSizeOptions = {12, 24, 48, 96}; // 默认每页条数选项

  // UI控件（可通过QSS定制）
  QLabel *m_totalLabel = nullptr;        // 总条数标签
  QLabel *m_pageSizeLabel;               // 保存“每页显示”标签的指针
