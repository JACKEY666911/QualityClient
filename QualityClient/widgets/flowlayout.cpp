...
{
    Q_Q(const FlowLayout);  // 安全获取q_ptr，避免空指针
    if (!q) {
        return -1;
    }
    ...
    if (m_center && !q->m_itemList.isEmpty()) {
...
FlowLayout::FlowLayout(QWidget *parent, int margin, int hSpacing, int vSpacing)
    : QLayout{parent},d_ptr(new FlowLayoutPrivate(hSpacing, vSpacing))
{
...
{
    return d_ptr->doLayout(QRect(0, 0, width, 0), true);
}
